...
let locationJsonStartAlreadySeen = false;
let locationJsonChunkCached = "";
const locationJsonStartExpected = `locations" : [ `;
let skipper = 0;
let noBoxOnThisDayCounter = 0;
let boxes = [
  {
    id: 0,
    name: "Koblenz",
...
class App extends Component {
  ...
  render() {
    return (
      <div className="App">
        ...
        <h1>{Math.round(this.state.percentageFileRead * 1000) / 10} %</h1>
  ...
  showChart() {
    ...
    boxes.forEach(box => {
      ...
      if (box.name !== "Koblenz") {
        noKoblenz += box.counter;
      } else {
        chartData.push({ x: box.name, y: box.counter });
      }
    });
    //chartData.push({ x: "Somewhere else", y: noBoxOnThisDayCounter });
    chartData.push({ x: "Not Koblenz", y: noKoblenz + noBoxOnThisDayCounter });
    this.setState({
      chartData: chartData
    });
    console.log(dates);
  ...
  onChangeHandler = event => {
    ...
    this.parseFile(event.target.files[0], (res, percentage) => {
      if (percentage - lastPercentage > 0.0003) {
        this.setState({
          percentageFileRead: percentage
        })
        lastPercentage = percentage;
      }
      this.parseLocationJson(res);
    })
  }

  parseLocationJson(jsonChunk) {
    jsonChunk = locationJsonChunkCached.concat(jsonChunk);

    //find starting position of whole json location entries
    if (!locationJsonStartAlreadySeen) {
      let startingIndex = jsonChunk.indexOf(locationJsonStartExpected);
      if (startingIndex !== -1) {
        //starting position found, remove it and set entry so that we will not search again for it
        jsonChunk = jsonChunk.substring(startingIndex + locationJsonStartExpected.length);
        this.setState({
          locationJsonStartAlreadySeen: true
        });
      } else {
        //when its not found, maybe we need to wait for another chunk
        this.setState({
          locationJsonChunkCached: jsonChunk
        });
      }
    }
    ...
    //loop as long as we find new entries
    do {
      let indexOpeningBracket = jsonChunk.indexOf('{');
      indexClosingBracket = -1;
      let openNestedBrackets = 0;
      if (indexOpeningBracket === -1) {
        //no opening found, maybe we need to wait for another chunk
        this.setState({
          locationJsonChunkCached: jsonChunk
        });
      } else {
        //loop through the chunk. If we find an opening bracket, we increment openNestedBrackets
        //if we find a closing bracket, we decrease openNestedBrackets
        //if openNestedBracket is back to the top level, we found the end of the entry
        for (let index = indexOpeningBracket + 1; index < jsonChunk.length; index++) {
          const char = jsonChunk[index];
          switch (char) {
            case "{":
              openNestedBrackets++;
              break;
            case "}":
              openNestedBrackets--;
              if (openNestedBrackets === -1) {
                indexClosingBracket = index;
              }
        ...
        //extract the locationEntry and remove it from the chunk
        if (skipper > 5) {
          if (indexClosingBracket !== -1) {
            let locationEntry = JSON.parse(jsonChunk.substring(indexOpeningBracket, indexClosingBracket + 1));
            jsonChunk = jsonChunk.substring(indexClosingBracket + 1);
    ...
    } while (indexClosingBracket !== -1);
  }


  analyzeLocationEntry(locationEntry) {
    //test
  ...
  parseFile(file, callback) {
    ...
    var readEventHandler = function (evt) {
      if (evt.target.error == null) {
        ...
        percentage = offset / fileSize;
        callback(evt.target.result, percentage); // callback for handling read chunk
      } else {
